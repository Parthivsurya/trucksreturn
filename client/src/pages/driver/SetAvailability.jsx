...
import { Navigation, MapPin, ArrowRight, Calendar } from 'lucide-react';
...
export default function SetAvailability() {
  ...
  if (currentCityData) {
    markers.push({ lat: currentCityData.lat, lng: currentCityData.lng, type: 'truck', label: `📍 Current: ${currentCityData.name}` });
  }
  if (destCityData) {
    markers.push({ lat: destCityData.lat, lng: destCityData.lng, type: 'delivery', label: `🏠 Destination: ${destCityData.name}` });
  }
  if (currentCityData && destCityData) {
    routes.push({
      from: [currentCityData.lat, currentCityData.lng],
      to: [destCityData.lat, destCityData.lng],
      color: '#3b82f6',
      dashed: true,
    });
  }
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!currentCityData || !destCityData) {
      toast.error('Select both current and destination cities');
      return;
    }
    if (form.currentCity === form.destinationCity) {
      toast.error('Current and destination cannot be the same');
      return;
    }
  ...
  return (
    <div className="page-container">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="section-subtitle mb-8">Tell us where you are and where you're heading. We'll find loads along your route.</p>

        <div className="grid lg:grid-cols-2 gap-6">
          {/* Form */}
          <form onSubmit={handleSubmit} className="glass rounded-2xl p-6 space-y-6 h-fit">
            <div>
              <label className="flex items-center gap-2 text-sm font-medium text-gray-400 mb-2">
                <div className="w-3 h-3 rounded-full bg-blue-500" /> Current Location
              </label>
              <select
            ...
            <div className="flex justify-center">
              <div className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center">
                <ArrowRight size={18} className="text-gray-500 rotate-90" />
              </div>
            </div>

            <div>
              <label className="flex items-center gap-2 text-sm font-medium text-gray-400 mb-2">
                <div className="w-3 h-3 rounded-full bg-red-500" /> Return Destination
              </label>
              <select
            ...
            {currentCityData && destCityData && (
              <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20">
                <p className="text-sm text-blue-300">
                  ...
                  {form.currentCity} → {form.destinationCity}
                </p>
                <p className="text-xs text-blue-400/70 mt-1">
                  We'll find loads near this route within 50km radius
                </p>
              </div>
            )}
